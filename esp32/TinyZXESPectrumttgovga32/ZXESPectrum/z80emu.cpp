...
//        }
    ...
    emulate_next_instruction:
        ...
        if (gb_auto_delay_cpu == 1)
        {
          ...
          {//Reajusta cada 4 instrucciones,pero puedo de sobra con 1
           cpu_cur= micros();
           microsMedido= (cpu_cur-cpu_before);
           microsExacto= 20000 * elapsed_cycles / number_cycles; //70000 o 70938
...
//unsigned char delay_contention(unsigned int tstates)
unsigned char delay_contention(word address, unsigned int tstates)
{
 ...
 unsigned short int line = tstates / 224; //int line = tstates / 224;
 ...
 unsigned char halfpix = tstates % 224; //int halfpix = tstates % 224;
 if (halfpix >= 128) return 0;

 //int modulo = halfpix % 8;
 //return wait_states[modulo];
 return wait_states[(halfpix & 7)]; //Lo mismo que halfpix % 8;
}
